...
3. Create one schema-version-2 Task contract. Include the verification tier,
   ...
   digest, and explicit model and effort. `prepare` rejects a digest that does
   not equal the active snapshot; in a development snapshot that snapshot
   requires `ARKIRA_HOME_DEV=<repo>`, so resolve it with `bin/arkira harness
   resolve <repo>` under that same environment before writing the digest.
...
and one independent review. When a covering Executor receipt declares a
contract, Normal also uses the bound focused evidence and Elevated also uses
the named triggering surface proof; direct-host (receiptless) authoring uses
`surface_check: not-applicable` instead. Elevated also requires tree-bound
verified-gate authorization. Full local CI is explicit opt-in only.
...
--target merged|preview|production --provider codex|claude --session ID`. `SHA`
must be the exact 40 hex characters of the checked-out `HEAD` after the merge,
not an abbreviated form; the repo must have a clean working tree.
